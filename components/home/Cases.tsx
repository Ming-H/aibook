...
const cases = [
  {
    ...
    summary: '面向 APP 用户增长的增长决策系统，用 Multi-Agent 协同完成用户获取、激活与留存策略执行',
  ...
  },
  {
    ...
    summary: '面向销售场景的对话系统，完成意图路由、任务分发和多轮状态管理',
...
export function Cases() {
  return (
    <section className="bg-[linear-gradient(180deg,transparent,rgba(82,167,255,0.06),transparent)] px-5 py-14 sm:px-8 md:py-20">
      <div className="mx-auto max-w-section">
        <div className="mb-8 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl animate-on-scroll">
            <span className="eyebrow">Selected work</span>
            <h2 className="section-heading mt-5">项目展示必须像产品，而不是像简历附件</h2>
            <p className="section-copy mt-4 max-w-2xl">
              每个案例都包含真实场景、系统架构和可验证的交付结果，展示从想法到上线的完整落地过程
            </p>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
          <article className="card animate-on-scroll rounded-[28px] p-6 md:p-7">
              ...
              <div className="rounded-[24px] border border-[var(--border-default)] bg-[var(--background-secondary)] p-5">
                <div className="grid gap-3 sm:grid-cols-3">
                  {cases[0].metrics.map((metric) => (
                    <div
                      key={metric}
                      className="rounded-[20px] border border-[var(--border-default)] bg-[var(--background-primary)] px-4 py-5"
                    >
                ...
                <div className="mt-5 rounded-[20px] border border-[var(--border-default)] bg-[linear-gradient(135deg,rgba(102,231,213,0.08),transparent_60%)] p-5">
          ...
          <div className="grid grid-rows-2 gap-5">
            {cases.slice(1).map((item, index) => (
              ...
              >
                ...
                <h3 className="mt-4 text-xl font-semibold tracking-[-0.02em]">{item.name}</h3>
                <p className="mt-3 body-copy flex-1">{item.summary}</p>
